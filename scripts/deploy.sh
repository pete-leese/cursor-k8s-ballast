...
# with each service's values file from git and reconciles the `ballast` namespace.
#
...
for _ in $(seq 1 30); do
  kubectl get ns ballast >/dev/null 2>&1 && break
  sleep 2
done
kubectl -n ballast get pods 2>/dev/null || true
