...
# the StreamIngestCrashLooping alert to fire, then runs the ballast RCA engine
...
echo "==> Waiting for StreamIngestCrashLooping to fire (for: 1m)..."
for _ in $(seq 1 40); do
  if curl -fsS http://localhost:9090/api/v1/alerts 2>/dev/null \
      | grep -q '"alertname":"StreamIngestCrashLooping","[^}]*"state":"firing"' \
     || curl -fsS http://localhost:9090/api/v1/alerts 2>/dev/null \
      | grep -q 'StreamIngestCrashLooping'; then
    state=$(curl -fsS http://localhost:9090/api/v1/alerts | "$PY" -c "import sys,json;a=[x for x in json.load(sys.stdin)['data']['alerts'] if x['labels']['alertname']=='StreamIngestCrashLooping'];print(a[0]['state'] if a else 'none')")
    [ "$state" = "firing" ] && break
  fi
  sleep 3
done
...
"$PY" -m ballast.cli investigate \
  --service ingest --healthy-memory 128Mi \
