...
All five services (`ingest`, `transcode`, `playback`, `recommendations`, `catalog`)
are `Running` and `Ready`. `ingest` is the upstream every other service
depends on (see `topology.yaml`).

## 2. Ship the bad chart bump (a git commit ArgoCD syncs)

```bash
task break                    # opens PR: ingest 128Mi -> 16Mi (merge manually)
```

Branches from `main`, edits `deploy/services/ingest.values.yaml`, and opens a
...
```bash
kubectl -n demo get pods -l app=ingest
# STATUS: CrashLoopBackOff, RESTARTS climbing
kubectl -n demo describe pod -l app=ingest | grep -A3 'Last State'
# Reason: OOMKilled, Exit Code: 137
```

## 3. The alert fires

`StreamIngestCrashLooping` fires after `for: 1m`. Expose Prometheus:
...
```bash
.venv/bin/python -m ballast.cli investigate --service ingest --healthy-memory 128Mi
```
...
4. computes blast radius from `topology.yaml` — `transcode, catalog, recommendations, playback`;
5. recommends **forward_fix** (restore the one memory-limit field) over a full
   rollback that would re-roll `ingest` and disrupt five dependents.
...
kubectl -n demo get pods -l app=ingest   # ArgoCD syncs; back to Running/Ready
```
